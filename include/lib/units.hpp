 ...
 * @file units.hpp
 * @author Jamie, maddy
 * @brief Don't crash your Mars Orbiter: Coherent unit system based on SI
 */
#pragma once
...
template <typename Mass, typename Length, typename Time, typename Angle> class Quantity {
  ...
  public:
    ...
    constexpr inline double convertRounded(const Quantity unit) const {
      double converted = convert(unit);
      return std::round(converted * 1000) / 1000;
    }

    constexpr inline Quantity convert_to(const Quantity unit) const { return Quantity(value / unit.raw()); }
...
operator/(const double& x, const Q& rhs) {
  return Quantity<std::ratio_subtract<Zero, decltype(Q::dim.mass)>, std::ratio_subtract<Zero, decltype(Q::dim.length)>,
                  std::ratio_subtract<Zero, decltype(Q::dim.time)>, std::ratio_subtract<Zero, decltype(Q::dim.angle)>>(
      x / rhs.raw());
}
...
namespace units {
  ...
  template <isQuantity R, isQuantity Q> constexpr inline Quantity<
      std::ratio_multiply<decltype(Q::dim.mass), R>, std::ratio_multiply<decltype(Q::dim.length), R>,
  ...
  template <int R, isQuantity Q> constexpr inline Quantity<std::ratio_divide<decltype(Q::dim.mass), std::ratio<R>>,
  ...
  template <isQuantity Q> constexpr inline Quantity<std::ratio_divide<decltype(Q::dim.mass), std::ratio<2>>,
  ...
  template <isQuantity Q> constexpr inline Quantity<std::ratio_divide<decltype(Q::dim.mass), std::ratio<3>>,
...
QUANTITY_LIT(LinearVelocity, ftps, ft / sec);
...
QUANTITY_NEW(Beta, beta, 0, -2, 0, 0);
QUANTITY_NEW(Zeta, zeta, 0, 0, 0, -1);

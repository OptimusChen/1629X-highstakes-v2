...
namespace util {
    ...
    float calculate_longest_angle(float target, float current) {
        ...
        // Calculate the longest angular difference
        float longest_diff = (diff > 180) ? diff : (360 - b) + s;

        // Directional multiplier function (determines whether to turn clockwise or counterclockwise)
        auto dir_to_spin = [](float target, float current) {
            // If the angular difference is less than 180 degrees, turn counterclockwise, otherwise clockwise
            return (fmod((target - current + 360), 360) < 180) ? -1 : 1;
        };

        // Apply the directional multiplier
        return longest_diff * dir_to_spin(target, current);
    }

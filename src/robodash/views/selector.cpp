...
static void mcl_cb(lv_event_t *event) {
	lv_obj_t *obj = lv_event_get_target(event);
	rd::Selector *selector = (rd::Selector *)lv_obj_get_user_data(obj);
	if (selector == nullptr) return;

	auto routine = selector->selected_routine;
	if (routine == nullptr) return;

	const char *routine_name = routine->name.c_str();

	selector->robot->set_pose(routine->startX, routine->startY, routine->startTheta);
	selector->robot->poseSet = true;
	selector->robot->initialize_particle_filter();
	selector->robot->calibrate();

	char label_str[strlen(routine_name) + 22];
	sprintf(label_str, "Calibrating routine:\n%s", routine_name);
	lv_label_set_text(selector->selected_label, label_str);
	lv_obj_align(selector->selected_label, LV_ALIGN_CENTER, 120, 0);	
}

void rd::Selector::coords_cb(lv_event_t *event) {
	lv_obj_t *obj = lv_event_get_target(event);
	rd::Selector *selector = (rd::Selector *)lv_obj_get_user_data(obj);
	if (selector == nullptr) return;
	...
	// Retrieve the robot's location (assuming you have a function to get the robot's location)
	auto pose = selector->robot->get_pose();

	std::cout << "x: " << pose.x << ", y: " << pose.y << ", heading: " << util::degrees(pose.theta) << std::endl;

	// Display the location on the screen
	char label_str_2[std::to_string(pose.x).length() + std::to_string(pose.y).length() + std::to_string(util::degrees(pose.theta)).length() + 22];
	sprintf(label_str_2, "x: %.2f, y: %.2f, heading: %.2f", pose.x, pose.y, util::degrees(pose.theta));
	lv_label_set_text(selector->selected_label, label_str_2);
	lv_obj_align(selector->selected_label, LV_ALIGN_CENTER, 120, 0);
}

// ============================== Constructor ============================== //

rd::Selector::Selector(lib::Robot* robot, std::vector<routine_t> autons) : Selector(robot, "Auton Selector", autons) {}

rd::Selector::Selector(lib::Robot* robot, std::string name, std::vector<routine_t> new_routines) {
	this->name = name;
	this->robot = robot;
	...
	lv_obj_t *mcl_btn = lv_list_add_btn(routine_list, NULL, "Calibrate");
	lv_obj_add_event_cb(mcl_btn, &mcl_cb, LV_EVENT_PRESSED, nullptr);
	lv_obj_set_user_data(mcl_btn, this);
	lv_obj_add_style(mcl_btn, &style_list_btn, 0);
	lv_obj_add_style(mcl_btn, &style_list_btn_pr, LV_STATE_PRESSED);

	lv_obj_t *coords_btn = lv_list_add_btn(routine_list, NULL, "Check Coords");
	lv_obj_add_event_cb(coords_btn, &coords_cb, LV_EVENT_PRESSED, nullptr);
	lv_obj_set_user_data(coords_btn, this);
	lv_obj_add_style(coords_btn, &style_list_btn, 0);
	lv_obj_add_style(coords_btn, &style_list_btn_pr, LV_STATE_PRESSED);
